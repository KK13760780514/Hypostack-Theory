...
| `exploratory` | `challenge` 或未达统计阈值 | 按预注册完成，结果为 challenge 或未达统计阈值，需进一步分析。 |
| `needs_review` | 任意（校验未通过） | 格式、统计或可复现性尚未通过检查，需修正后重新提交。 |
...
| `exploratory` | `challenge` or below statistical threshold | Completed per preregistration; the result is a challenge or below the statistical threshold; further analysis needed. |
| `needs_review` | Any (validation failed) | Form, statistical, or reproducibility checks have not passed; resubmit after correction. |

There is also a historical level:
