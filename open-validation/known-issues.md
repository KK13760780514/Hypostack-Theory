...
**修正方案（参数调优 + 无对照规则修订）**：
- X2_SCALE: 10 -> 20（条件数从 ~100 提升至 ~400，Adam 优势放大）
...
**验证结果**：12/12 种子通过，p=2.4e-4，结论 **support**。修正后 Adam 平均 S=188.53，最优 SGD 平均 S=345.97，ratio=0.545（省力约 50%），效应显著增强。

**待社区决策**：
1. 条件数从 1e2 提升到 4e2 是否合理？这是否人为制造了有利于 Adam 的场景？
...
**验证结果**：在全部 8 个温度下，E_eff_A < E_eff_B（如 T=300K 时 E_eff_A≈31.7 kJ/mol < E_eff_B=55 kJ/mol），玄叠论预测路径 A，与 Arrhenius 动力学结果一致。
